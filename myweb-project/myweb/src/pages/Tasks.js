...
import { Plus, X, Check, Trash2, Flag, Calendar, Tag, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function Tasks() {
  ...
  const catStats = useMemo(() => {
    return CATEGORIES.map(cat => {
      ...
      return { ...cat, total: catTasks.length, done, pct: catTasks.length ? Math.round((done / catTasks.length) * 100) : 0 };
    });
  }, [tasks]);
